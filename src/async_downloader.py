...
M3U8_RES = re.compile(r"#EXT-X-STREAM-INF:.+RESOLUTION=(?P<width>\d+)x(?P<height>\d+).+")
...
async def get_m3u8_url(anime_id: int, episode: int, lang: str) -> M3U8:
    ...
    if data is None:
        raise ValueError(f"Language {lang} is not available for anime {anime_id} and episode {episode}")
...
async def get_available_qualities(url: str) -> list[Quality]:
    ...
    for line in lines:
        if line.startswith("#EXT") and (match := M3U8_RES.search(line)):
            width, height = map(int, match.groups())
            qualities.append(Quality(url=next(lines), width=width, height=height))
    return qualities


async def download_form_m3u8(url: str, output: str) -> tuple[asyncio.subprocess.Process, float]:
    ...
    process = await asyncio.create_subprocess_exec(*args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
